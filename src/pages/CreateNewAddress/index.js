...
import { loading as loadingAddAddress } from '../../redux/actions/v2/account/CreateShippingAddressAction'
import {
    cityAction,
...
const CreateNewAddress = ({ navigation }) => {
    const dispatch = useDispatch()
    const { errorMessage: error } = useSelector(
        state => state.CreateShippingAddressReducer
    )
    ...
    const success = () => showSuccess('Penambahan alamat baru berhasil.')

    const addAddressHandler = () => {
        dispatch(
            CreateShippingAddressAction(form, navigation, { success, setForm })
        )
    }

    useEffect(() => {
        dispatch(provinceAction())
        dispatch(loadingAddAddress())
    }, [])

    return (
        <View style={styles.container}>
            <TopbarHeader title="Tambah Alamat Baru" goBack={goBack} />
            <View style={styles.content}>
                <ScrollView
                    ...
                    <ButtonOpacity title="Tambah" onPress={addAddressHandler} />
                    <Gap height={50} />
                </ScrollView>
            </View>
        </View>
